...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,success: function(res){
          typeof options.dataRender === 'function' && options.dataRender(res);
  ...
  Class.prototype.syncCheckAll = function(){
    ...
    if(table.checkStatus(that.key).isAll){
      if(!checkAllElem.last()[0].checked){
    ...
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){
      ...
      if(isAll){
        childs.each(function(i, item){
          ...
          var dataIndex = $(item).parents('tr').first().data('index');
  ...
  table.checkStatus = function(id){
    ...
    layui.each(data, function(i, item){
      if(item.constructor === Array||item.checkDisabled){
